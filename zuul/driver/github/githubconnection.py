...
class GithubRateLimitHandler:
    ...
    def _handle_rate_limit(self, response):
        ...
        wait_time = max(int(reset) - int(time.time()) + 1, 0)
...
class GithubConnection(ZKChangeCacheMixin, ZKBranchCacheMixin, BaseConnection):
    ...
    def _updateChange(self, change, event, pull):
        ...
        # Similarly, never overwrite merge_commit_sha which is set by
        # mergePull.
        if change.is_merged and not change.merge_commit_sha:
