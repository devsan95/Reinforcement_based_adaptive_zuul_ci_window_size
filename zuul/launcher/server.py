...
class LabelNodeMapping:
    ...
    def getNodesForLabel(self, label):
        return list(self.label_to_nodes[label].values())
...
class Launcher:
    ...
    _stats_interval = 30
    # If subnodes do not appear within this window, assume a crash and
    # clean them up.
    SUBNODE_SYNC_TIME = 300
    ...
    def _selectProviders(self, request, request_unassigned_nodes, messages):
        ...
        if main_provider_name:
            for main_provider in providers:
                ...
                messages.append(
                    f"Unable to find provider {main_provider_name}; ignoring")
    ...
    def _registerStaticNodes(self, provider):
        ...
        with self.createZKContext(None, self.log) as ctx:
            for static_node in provider.nodes.values():
                ...
                desired_slots = max(int(getattr(label, 'slots', 1) or 1), 1)
                if node:
                    # Static slot-hosts are not resized in place. When the
                    # configured slots change, deregister so the next pass
                    # recreates the host + READY subnodes.
                    existing = list(getattr(node, 'subnodes', None) or [])
                    have = len(existing)
                    want = desired_slots if desired_slots > 1 else 0
                    if have == want and node.state not in (
                            node.State.FAILED, node.State.TEMPFAILED,
                            node.State.USED, node.State.HOLD):
                        continue
                    self.log.info(
                        "Recreating static node %s "
                        "(subnodes have=%s want=%s state=%s)",
                        node, have, want, node.state)
                    for sub_id in existing:
                        if sub := self.api.getProviderNode(sub_id):
                            self._bestEffortLockAndSetNodeState(
                                ctx, sub, sub.State.DEREGISTER, self.log)
                    self._bestEffortLockAndSetNodeState(
        ...
        with self.createZKContext(None, self.log) as ctx:
            for node in self.api.getProviderNodes():
                ...
                if node.uuid in provider.nodes:
                    continue
                # Keep slot subnodes while their static main node is still
                # configured. Reconfigure otherwise churns them into a stuck
                # BUILDING state and gate jobs never get READY nodes.
                main_id = getattr(node, 'main_node_id', None)
                if main_id and main_id in provider.nodes:
                    continue
    ...
    def _requestNode(self, label, request, provider, log, ctx,
        ...
        args = dict(
            ...
            # Set the reuse information on the node. This is used only for
            # informational purposes (the actual decision about reuse will
            # happen later).
            reuse=label.reuse,
        )
        ...
        if label.slots and label.slots > 1:
            for slot in range(max(label.slots, 0)):
                subnodes.append(uuid.uuid4().hex)
            args['subnodes'] = subnodes
        else:
            args.update(request_args)
        # Tags should definitely be set on the main node regardless;
        # they are unnecessary on subnodes.
        args['tags'] = provider.getNodeTags(
            self.system.system_id, label, node_uuid, request)
        node_class = provider.driver.getProviderNodeClass()
        node = node_class.new(ctx, **args)
        log.debug("Requested node %s", node)
        retnode = node

        for slot, subnode_uuid in enumerate(subnodes):
            if slot == 0:
                merged_subnode_args = {}
            ...
            subnode = self._createSubNode(
                ctx, node, merged_subnode_args, slot, subnode_uuid)
        ...
        if not subnodes and request is not None:
            # Assign the main (only) node
            node.assign(ctx, **assign_args)
        return retnode

    def _createSubNode(self, context, node, args, slot, subnode_uuid):
        args['uuid'] = subnode_uuid
    ...
    def _checkNodeCreate(self, node, log):
        if isinstance(node, SubnodeProviderNode):
            if self.api.getProviderNode(node.main_node_id) is None:
                self.wake_event.set()
                now = time.time()
                if (now - node.request_time > self.SUBNODE_SYNC_TIME):
                    raise ProviderNodeError(
                        "Main node not present")
                return
            # After static reuse/delete, subnodes can land in BUILDING with
            # no create work left. Promote them back to READY via reuse.
            if node.state == node.State.BUILDING:
                main = self.api.getProviderNode(node.main_node_id)
                if main and main.state in (
                        main.State.SLOT_HOST, main.State.READY):
                    self._checkNodeReuse(node, log)
            return
        ...
        with self.createZKContext(node._lock, self.log) as ctx:
            with node.activeContext(ctx):
                ...
                if self._checkNodescanRequest(node, log):
                    if node.subnodes:
                        # Handle a race where we finish building the
                        # main node very quickly (ie, static driver)
                        # before we have completely written all the
                        # subnode records.
                        for subnode_id in node.subnodes:
                            if not self.api.getProviderNode(subnode_id):
                                log.warning("Subnode %s does not exist",
                                            subnode_id)
                                now = time.time()
                                if (now - node.request_time >
                                    self.SUBNODE_SYNC_TIME):
                                    raise ProviderNodeError(
                                        "Subnodes not present at creation")
                                self.wake_event.set()
                                return
