...
class PipelineManager(metaclass=ABCMeta):
    ...
    def __init__(self, sched, pipeline, tenant):
        ...
        self._change_cache = {}
        # Current ZK context when the pipeline is locked (per-thread)
        self._current_context_local = threading.local()
    ...
    def current_context(self):
        return getattr(self._current_context_local, 'value', None)

    @current_context.setter
    def current_context(self, value):
        if value is None:
            if hasattr(self._current_context_local, 'value'):
                del self._current_context_local.value
        else:
            self._current_context_local.value = value
    ...
    def prepareItem(self, item):
        ...
        if build_set.merge_state == build_set.NEW:
            if item.live or item.updatesConfig():
                ...
                # We only need to request config/dirs files if any of
                # the changes updates the config.
                if item.updatesConfig():
                    ...
                    for merger_item in item.current_build_set.merger_items:
                        ...
                        tpc = tenant.project_configs.get(
                            project.canonical_name)
                        if tpc:
                            extra_config_files.update(tpc.extra_config_files)
                            extra_config_dirs.update(tpc.extra_config_dirs)

                    files = ['zuul.yaml', '.zuul.yaml', *extra_config_files]
                    dirs = ['zuul.d', '.zuul.d', *extra_config_dirs]
                ready = self.scheduleMerge(item, files=files, dirs=dirs)
    ...
    def _onMergeCompleted(self, event, build_set):
        ...
        with build_set.activeContext(self.current_context):
            ...
            if event.merged and event.files:
    ...
    def reportItem(self, item):
        ...
        if self.changes_merge:
            ...
            if not (succeeded and merged):
                ...
                from zuul.rl_window import adjust_window_after_cycle
                adjust_window_after_cycle(change_queue, succeeded=True)
                log.debug("%s window size adjusted to %s",
    ...
    def formatStatusJSON(self, websocket_url=None):
        ...
        try:
            from zuul.rl_window import (
                get_pipeline_status_json,
                get_queue_status_json,
            )
            rl_pipeline = get_pipeline_status_json(
                self.tenant.name, self.pipeline.name)
            if rl_pipeline is not None:
                j_pipeline['rl_window'] = rl_pipeline
        except Exception:
            pass
        for queue in self.state.queues:
            if not queue.queue:
            ...
            try:
                j_queue.update(get_queue_status_json(
                    self.tenant.name, self.pipeline.name, queue))
            except Exception:
                pass
            ...
            if j_changes:
                j_queue['heads'].append(j_changes)
        if j_pipeline.get('rl_window'):
            rl_by_name = {
                q.get('name'): q
                for q in j_pipeline['rl_window'].get('queues', [])
            }
            for j_queue in j_queues:
                rl_q = rl_by_name.get(j_queue.get('name'))
                if rl_q is None:
                    continue
                j_queue.setdefault(
                    'rl_recommended_window', rl_q.get('recommended_window'))
                j_queue.setdefault('rl_action_delta', rl_q.get('action_delta'))
                j_queue.setdefault('rl_mode', j_pipeline['rl_window'].get('mode'))
                j_queue['rl_current_window'] = j_queue.get('window')
        return j_pipeline
